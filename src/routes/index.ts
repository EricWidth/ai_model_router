...
import { createSemanticRouter } from './semantic'
...
export function registerRoutes(app: Application, ctx: AppContext): void {
  ...
  app.use('/v1', accessAuth(() => ctx.config.server.accessApiKey), createSemanticRouter(ctx))
  app.use('/v1/tasks', accessAuth(() => ctx.config.server.accessApiKey), createTasksRouter(ctx))
